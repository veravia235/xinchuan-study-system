...
st.title("🎓 新传考研 5W 智能全能学习系统 [免流无错版]")
st.caption("集成 5W深度拆解、新闻史编年体、热点实战阅卷与全云端高效知识感应引擎")
...
with st.sidebar:
    ...
    if uploaded_files and st.button("🚀 1. 快速抽取资料文本"):
        all_text = []
        ...
        if all_text:
            # 将合并后的文本保存，文件太大时截取前部分防止云端显存溢出
            st.session_state.raw_notes_text = "\n".join(all_text)[:150000]
            status_text.text("✅ 资料文本暂存成功！")
            st.toast("资料已加载，可以开始第2步进化了！")
        else:
            status_text.text("❌ 未能提取有效文本")

    # 全云端知识进化核心引擎
    if st.session_state.raw_notes_text:
        st.markdown("---")
        st.header("🧠 2. 智能深度重构")
        st.caption("让 AI 智能扫描暂存资料，并瞬间重构右侧的【5W表格】和【新闻史时间轴】")
        if st.button("🪄 融合资料至系统功能面"):
            if not MY_DEEPSEEK_API_KEY:
                st.error("请先填入 API Key 才能调用进化引擎！")
            else:
                with st.spinner("AI 正在深度重构系统数据，这需要短短十几秒..."):
                    try:
                        client = OpenAI(api_key=MY_DEEPSEEK_API_KEY, base_url=base_url)
                        
                        prompt = f"请精读以下用户上传的复习资料：\n{st.session_state.raw_notes_text[:20000]}\n\n" + """
                        ...
                        st.error(f"提取融合失败，请稍后重试: {e}")

# ==========================================
# 3. 默认内置的基础数据库 (未上传资料时显示)
# ==========================================
DEFAULT_THEORIES = {
    "议程设置理论": {
        "5w": ["麦库姆斯/肖 (延续控制分析)", "媒介议程影响公众议程 (属性/网络议程)", "传统大众媒介->算法分发时代", "非完全被动受众，认知受框架限制", "认知层面的强效果论复兴"],
        "clues": ["源于李普曼的‘拟态环境’概念", "基本观点是媒介不能决定人们怎么想，但能决定人们想什么", "第二层侧重属性议程，第三层侧重网络议程建构"]
    },
    "沉默的螺旋": {
...
with tab3:
    ...
    t1.error("🔥 【名词解释转化】\n请阐释该热点中的底层概念核心。")
    t2.warning("🔥 【简答题转化】\n请简述该现象的产生机制及背后多方主体的利益合谋。")
    t3.success("🔥 【论述题转化】\n结合马克思主义新闻观、批判学派异化视角，论述该现象的现实危害。")
    
    student_ans = st.text_area("在此粘贴或撰写你的大纲/全文：", height=150)
    if st.button("提交答卷进行机器判分"):
        if not MY_DEEPSEEK_API_KEY: st.error("密钥缺失")
        elif len(student_ans) < 15: st.warning("字数不足")
        else:
            with st.spinner("阅卷组正在判卷..."):
                try:
                    client = OpenAI(api_key=MY_DEEPSEEK_API_KEY, base_url=base_url)
                    prompt = f"请作为新闻传播学考研阅卷组长，针对热点【{current_hot}】，评判考生的作答：\n{student_ans}\n给出满分30分下的明确得分与高分示范大纲。行文中视具体语境使用十分、极度、非常等细化梯度表达，绝对禁止使用‘极其’一词。"
                    response = client.chat.completions.create(model="deepseek-chat", messages=[{"role": "user", "content": prompt}])
                    st.markdown("### 📋 官方详细判卷报告")
                    st.write(response.choices[0].message.content)
                except Exception as e: st.error(f"判卷失败: {e}")
...
with tab4:
    ...
    ai_mode = st.radio("配置导师执教风格：", ["☕ 围炉煮茶 (温和基础答疑)", "🌪 苏格拉底风暴 (多轮反问)", "🎯 私库魔鬼突击抽题 (基于你解析过的资料)"], horizontal=True)
    
    st.write("---")
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]): st.write(msg["content"])
            
    u_input = st.chat_input("向导师发问...")
    if u_input:
        with st.chat_message("user"): st.write(u_input)
        st.session_state.messages.append({"role": "user", "content": u_input})
        
        context_str = ""
        if st.session_state.raw_notes_text and "私库" in ai_mode:
            context_str = f"\n【参考以下用户笔记片段进行针对性抽题拷问】:\n{st.session_state.raw_notes_text[:15000]}"
            
        sys_prompt = "你是辅导新传考研的资深导师。分析理论应用 5W 框架。绝对禁用‘极其’一词，视语境使用‘非常’‘十分’‘相当’‘极度’等细化梯度词替代。"
        if "苏格拉底" in ai_mode: sys_prompt += "不要直接给答案，抛出更深层的提问。"
        elif "私库" in ai_mode: sys_prompt += "请根据用户笔记片段挑选考点进行压迫感抽题。"
            
        payload = [{"role": "system", "content": sys_prompt + context_str}]
        payload.extend(st.session_state.messages[-6:])
        ...
        try:
            ...
            with st.chat_message("assistant"):
                box = st.empty()
                reply = ""
                res_stream = client.chat.completions.create(model="deepseek-chat", messages=payload, stream=True)
